...
multi.impute <- function(data, conditions, nb.imp = NULL, method, parallel = F){
  ...
  if (method == "RF") {
    if (parallel) {
      data.array <- simplify2array(foreach(iforeach =  1:nb.imp,
                                           .errorhandling = 'remove', .verbose = T) %dopar% 
  ...
  if (method == "MLE") {
    if (parallel) {
      data.array <- simplify2array(foreach(iforeach =  1:nb.imp,
                                           .errorhandling = 'remove', .verbose = T) %dopar% 
                                     impute.mle(tab = data, conditions = conditions))
    }
    else {
      data.array <- simplify2array(lapply(1:nb.imp, function(aaa) {
  ...
  if (method == "PCA") {
    if (parallel) {
      data.array <- simplify2array(foreach(iforeach =  1:nb.imp,
                                           .errorhandling = 'remove', .verbose = T) %dopar% 
                                     impute.PCA(tab = data, conditions = conditions))
    }
    else {
      data.array <- simplify2array(lapply(1:nb.imp, function(aaa) {
